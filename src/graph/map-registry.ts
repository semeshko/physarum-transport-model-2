...
export function createTransportWorkspaceRegistry(dataset: GISDataset, graph: TransportGraph, gisVisibility: GISLayerVisibility, graphVisibility: GraphLayerVisibility): LayerRegistry {
  ...
  const nodeData = {
    type: "FeatureCollection",
    features: graph.nodes.map((node) => ({ type: "Feature", id: node.id, properties: { nodeId: node.id }, geometry: { type: "Point", coordinates: node.position } })),
  } as unknown as MapGeoJSON;
  ...
  return createLayerRegistry(
    ...
    [
      ...gisRegistry.layers,
      { id: "transport-graph-edges", type: "line", source: "transport-graph-edges-source", visible: graphVisibility.edges, paint: { "line-color": "#ff4fd8", "line-width": 2, "line-opacity": 0.9, "line-dasharray": [2, 1.5] } },
