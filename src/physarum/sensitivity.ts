...
export const SENSITIVITY_MODEL_VERSION = "physarum-hill-euler-v2";
...
/** Fine sweep around the n=1 distributed / n>1 winner-takes-all transition
 * (Task 17 N REGIME STUDY). Run across multiple topologies, not just one
 * parallel-edge toy graph - see NARROW_TRANSITION_FIXTURES below. */
export const HILL_EXPONENTS_FINE = [1.0, 1.025, 1.05, 1.075, 1.1, 1.15, 1.2, 1.25, 1.5, 2.0] as const;
export const NARROW_TRANSITION_FIXTURES = ["parallel-corridors", "three-corridors", "shared-trunk", "grid"] as const;
export const HILL_K_VALUES = [0.1, 0.5, 1, 2, 5] as const;
/** kappa sweep for demand-normalized mode (Task 17 KAPPA STUDY). */
export const KAPPA_VALUES = [0.05, 0.1, 0.25, 0.5, 1, 2] as const;
/** The exploratory n proposed for Design Mode (see n-fine study) - kappa
 * must be re-swept here, not just at the legacy n=2 selective baseline,
 * since kappa's effect is coupled to n. */
export const DESIGN_REGIME_HILL_EXPONENT = 1.05 as const;
...
export const DEMAND_MAGNITUDES = [0.1, 0.5, 1, 2, 5, 10, 100] as const;
...
export type SensitivityObservation = { readonly label: string; readonly fixtureId: SensitivityFixtureId; readonly costs: readonly number[]; readonly sourceMagnitude: number; readonly parameters: PhysarumParameters; readonly scaleMode: "absolute" | "demandNormalized"; readonly qRef: number; readonly effectiveHillK: number; readonly convergence: { readonly converged: boolean; readonly reason: string | null; readonly iterations: number; readonly maxDeltaD: number; readonly kirchhoffResidual: number }; readonly flow: DistributionMetrics; readonly conductivity: DistributionMetrics; readonly activeEdgeFraction: number; readonly edgeFlows: Readonly<Record<string, number>>; readonly edgeConductivities: Readonly<Record<string, number>>; readonly runtimeMilliseconds: number | null };
...
export function createSensitivityReport(): PhysarumSensitivityReport {
  ...
  for (const magnitude of DEMAND_MAGNITUDES) observations.push(observeSensitivity(createSensitivityFixture("parallel-corridors",1.1,magnitude),{...robust,demandScaleKappa:1},`demand-normalized:${magnitude}`));
  ...
  for (const id of NARROW_TRANSITION_FIXTURES) for (const hillExponent of HILL_EXPONENTS_FINE) observations.push(observeSensitivity(createSensitivityFixture(id),{...robust,hillExponent},`n-fine:${id}:${hillExponent}`));
  for (const kappa of KAPPA_VALUES) observations.push(observeSensitivity(createSensitivityFixture("parallel-corridors"),{...robust,demandScaleKappa:kappa},`kappa:${kappa}`));
  for (const id of NARROW_TRANSITION_FIXTURES) for (const kappa of KAPPA_VALUES) observations.push(observeSensitivity(createSensitivityFixture(id),{...robust,hillExponent:DESIGN_REGIME_HILL_EXPONENT,demandScaleKappa:kappa},`kappa-at-design-n:${id}:${kappa}`));
