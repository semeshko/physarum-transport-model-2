...
import { createCostQAReport, serializeCostQAReport } from "./qa";
import calibrationFixture from "../data/cost-calibration.json";
...
describe("profile generalized cost", () => {
  ...
  it.each(["pedestrian", "bicycle", "motor"] as const)("keeps surface and smoothness factors monotonic for %s", (profileId) => { const values = ["excellent", "intermediate", "bad", "very_bad", "horrible"].map((smoothness, index) => applyGeneralizedCosts(network(profileId, [feature(String(index), profileId === "bicycle" ? "path" : profileId === "motor" ? "track" : "footway", { smoothness })]).profiled).edges[0].cost.conditionFactor); expect(values).toEqual([...values].sort((a, b) => a - b)); });
  it("uses tracktype only when surface and smoothness are absent", () => { const fallback = applyGeneralizedCosts(network("bicycle", [feature("g", "track", { tracktype: "grade5" })]).profiled).edges[0].cost; const explicit = applyGeneralizedCosts(network("bicycle", [feature("g", "track", { tracktype: "grade5", surface: "asphalt" })]).profiled).edges[0].cost; expect(fallback).toMatchObject({ conditionSource: "tracktype-fallback", tracktypeFactor: 1.8, conditionFactor: 1.8 }); expect(explicit).toMatchObject({ conditionSource: "surface", surfaceFactor: 1, tracktypeFactor: 1, conditionFactor: 1 }); });
  ...
  it("produces a deterministic serializable QA report", () => { const costed = applyGeneralizedCosts(network("motor", [feature("explicit", "primary", { maxspeed: "50", surface: "asphalt" }), feature("fallback", "track", { tracktype: "grade3" }, 1)]).profiled); const report = createCostQAReport(costed); expect(report.counts).toMatchObject({ edges: 2, explicitSpeed: 1, fallbackSpeed: 1, surfaceTagged: 1, tracktypeTagged: 1 }); expect(serializeCostQAReport(report)).toBe(serializeCostQAReport(createCostQAReport(costed))); expect(JSON.parse(serializeCostQAReport(report))).toEqual(report); });
  it.each(["pedestrian", "bicycle", "motor"] as const)("costs the committed calibration fixture for %s", (profileId) => { const graph = buildTransportGraph(ingestGeoJSON(calibrationFixture, options)); const costed = applyGeneralizedCosts(applyTransportProfile(graph, profileId)); expect(costed.edges.length).toBeGreaterThan(0); expect(costed.edges.every((item) => item.cost.generalizedCostSeconds > 0 && item.cost.speedEvidence.length > 0)).toBe(true); });
});
